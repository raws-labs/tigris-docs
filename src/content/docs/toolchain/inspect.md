...
budget, use [`tigris analyze`](/toolchain/analyze/).
...
compiler recorded. This is the interface [`tigris run`](/toolchain/run/)
...
- It needs no native library, so it works on every platform `tigris-ml`
  installs on. See [Installation](/getting-started/installation/).
