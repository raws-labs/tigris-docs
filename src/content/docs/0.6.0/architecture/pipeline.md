...
description: How TiGrIS compiles an ONNX model into a .tgrs execution plan
  through seven stages, ending in a binary the C99 runtime runs directly on the
  target device.
sidebar:
  order: 310
slug: 0.6.0/architecture/pipeline
---
...
* **birth\_step**: the execution step of the op that produces the tensor.
* **death\_step**: the execution step of the last op that consumes the tensor.
